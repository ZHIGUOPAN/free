...
RUN echo '000777' | vncpasswd -f > $HOME/.vnc/passwd
...
RUN echo 'whoami ' >>/000777.sh

RUN echo 'cd ' >>/000777.sh

RUN echo "su -l -c 'vncserver :2000 -geometry 1360x768' "  >>/000777.sh

RUN echo 'cd /noVNC-1.2.0' >>/000777.sh

RUN echo './utils/launch.sh  --vnc localhost:7900 --listen 8900 ' >>/000777.sh

RUN chmod 755 /000777.sh
